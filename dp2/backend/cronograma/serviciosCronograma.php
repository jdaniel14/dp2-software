...
function CR_getActividadesMovil(){
	...
	echo '<!DOCTYPE html PUBLIC "-//IETF//DTD HTML 2.0//EN">
				<HTML>
				   <HEAD>
			<meta http-equiv="refresh" content="0; url=http://200.16.7.112/trunk/dp2/static/cronograma/gantt/gantt_movil.html?idProyecto=' . $id . '"/>
				      <TITLE>
...
function CR_obtenerDetalleRecurso($idActividad,$idRecurso){
    ...
    try {
        ...
        while ($j = $stmt->fetch(PDO::FETCH_ASSOC)) {//queda por ver mienbros de equipo y el campo esta aceptado
            ...
            $rec = array("idrecurso" => $j["id_recurso"], "id" => "tmp_" . $contador, /*"resourceId" => $idRecurso*/ "idunidadmedida" => $j["id_unidad_medida"], "idTipoCosto" => $j["id_tipo_costo"], "descripcion_recurso" => $j["descripcion"], "costRate" => $j["costo_unitario_estimado"] + 0, "simbolo_unidad" => $j["simbolo_unidad"], "typeCost" => $j["descripcion_unidad"], "descripcion_moneda" => $j["descripcion_moneda"],  "value" => $j["cantidadEstimada"] + 0, "valueReal" => $j["cantidadReal"], "costRateReal" => $j["costo_unitario_real"], "descripcion_tipocosto" => $j["descripcion_tipocosto"]);
...
function CR_obteneListaDependenciaProyecto($idProyecto, $arreglo_critico,$arreglo_actividades_final) {//simil con lo hardcodeado ATP
    ...
    try {
        ...
        while ($j2 = $stmt2->fetch(PDO::FETCH_ASSOC)) {
        ...
        while ($j = $stmt->fetch(PDO::FETCH_ASSOC)) {//queda por ver mienbros de equipo y el campo esta aceptado
            if ($p == 0) {
                ...
                $rec = array("id_actividad" => $j["id_actividad"],"numero_fila" => $j["numero_fila"],"holgura_inicial"=>$arreglo_actividades_final[$ind]->lst-$arreglo_actividades_final[$ind]->est,"holgura_final"=>$arreglo_actividades_final[$ind]->let-$arreglo_actividades_final[$ind]->eet, "est" => $arreglo_actividades_final[$ind]->est,"lst" =>$arreglo_actividades_final[$ind]->lst,"eet" =>$arreglo_actividades_final[$ind]->eet,"let" =>$arreglo_actividades_final[$ind]->let,"nombre_actividad" => $j["nombre_actividad"], "marcado" => 0, "bloque" => $numbloque, "EsCritico" => $escritico, "numDias" => $j["dias"], "fecha_plan_inicio" => $listafechas[0], "fecha_plan_fin" => $listafechas[1], "fecha_actual_inicio" => $listafechas[2], "fecha_actual_fin" => $listafechas[3], "predecesores_id" => $j["predecesores"],/*"predecesores_id_real"*/ "predecesores"=> ($j["predecesores_id_concatenado"].""), "id_proyecto" => $j["id_proyecto"], "id_paquete_trabajo" => $j["id_paquete_trabajo"]); //id_paquete_trabajo
...
function CR_obtenerRecursosTotalProyecto($idProyecto) {
    ...
    try {
        ...
        $contador = 1;
        while ($j = $stmt->fetch(PDO::FETCH_ASSOC)) {//queda por ver mienbros de equipo y el campo esta aceptado
            $rec = array("idrecurso" => $j["id_recurso"], "id" => "tmp_" . $contador, "idunidadmedida" => $j["id_unidad_medida"], "name" => $j["descripcion"], "costRate" => $j["costo_unitario_estimado"], "simbolo_unidad" => $j["simbolo_unidad"], "typeCost" => $j["descripcion_unidad"], "descripcion_moneda" => $j["descripcion_moneda"]);
            array_push($listaRecursos, $rec);
            $contador++;
        }
