<?php
	...
	*/
	...
	function CO_consultarInfoProyecto($idProyecto) { //COMPLETO
		...
		AND D.ESTADO<>'ELIMINADO' AND C.ESTADO<>0
		AND
...
//			      echo '{"error":{"text":'. $e->getMessage() .'}}';
	...
	function CO_consultarListaRecursos($idProyecto) { //COMPLETO
		...
		SUM(IFNULL(A.COSTO_UNITARIO_ESTIMADO,0)) COSTO_PROM_SOLES
		FROM
		...
		LEFT JOIN CAMBIO_MONEDA Y ON A.ID_CAMBIO_MONEDA=Y.ID_CAMBIO_MONEDA
		LEFT JOIN UNIDAD_MEDIDA Z ON A.ID_UNIDAD_MEDIDA=Z.ID_UNIDAD_MEDIDA
		WHERE
		A.ID_PROYECTO= :idProyecto
		AND A.ESTADO<>'ELIMINADO' AND (C.ID_RECURSO IS NULL OR C.ESTADO<>0 )
		GROUP BY
		A.ID_RECURSO,
		A.ID_UNIDAD_MEDIDA,
		A.DESCRIPCION,
...
//			      echo '{"error":{"text":'. $e->getMessage() .'}}';
	...
	function CO_consultarListaActividades($idProyecto) { //COMPLETO
		...
		WHERE
		DATE_FORMAT(X.FECHA,'%Y%m%d')=DATE_FORMAT(SYSDATE(),'%Y%m%d')
		AND Z.ESTADO<>'ELIMINADO' AND C.ESTADO<>0
		AND
...
//			      echo '{"error":{"text":'. $e->getMessage() .'}}';
	...
	function CO_consultarListaPaquetes($idProyecto) { //COMPLETO
		...
		SUM(IFNULL(C.CANTIDADESTIMADA,0)*(IFNULL(D.COSTO_UNITARIO_ESTIMADO,0)*CAMBIO_A_SOL)) COSTO_PAQUETE_SOLES
		from 
		PROYECTO A 
		...
		AND D.ESTADO<>'ELIMINADO' AND Y.ID_COMPONENTE_PADRE IS NULL AND C.ESTADO<>0
		AND
		A.ID_PROYECTO= :idProyecto
		GROUP BY
		A.ID_PROYECTO,
	...
	function CO_obtenerPaquetesHijo(&$paquete) {
		if ($paquete != null) {
			...
			OR
			(DATE_FORMAT(X.FECHA,'%Y%m%d')=DATE_FORMAT(SYSDATE(),'%Y%m%d') AND Z.ID_ESTADO<>4 AND D.ESTADO<>'ELIMINADO' AND C.ESTADO<>0)
			)
			GROUP BY
			A.ID_PROYECTO,
			Y.ID_PAQUETE_TRABAJO,
