...
var currentDate = new Date();
		...
		$(function() {
		  ...
		  $(".ganttButtonBar div")
		          .append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;")
		          .append("<button onclick='openResourceEditor();' class='button'>Ver Recursos</button>")
...
//		      ret.tasks[i].start=ret.tasks[i].start+offset;
		...
		function openWorkingTimeEditor(){
		  ...
		  editor.append("<div class='row'><div class='tabbable'><ul class='nav nav-tabs'><li class='active'><a href='#pane1' data-toggle='tab'>Dias Libres</a></li><li><a href='#pane2' data-toggle='tab'>Week overrides</a></li></ul><button type='button' class='btn btn-default btn-lg' onclick='addWorkingTimeDay();'><span class='glyphicon glyphicon-plus-sign'></span> Agregar</button><button type='button' class='btn btn-default btn-lg' onclick='editWorkingTimeDay();'><span class='glyphicon glyphicon-pencil'></span> Editar</button><button type='button' class='btn btn-default btn-lg'><span class='glyphicon glyphicon-minus-sign'></span> Eliminar</button><div class='tab-content'><div id='pane1' class='tab-pane active'><table class='table table-stripped' style='margin-left:10px;'><thead><th></th><th>  Nombre</th><th>  Fecha</th></thead><tbody id='tableHolidayDay'></tbody></table></div><div id='pane2' class='tab-pane'><table class='table table-stripped' style='margin-left:10px;'><thead><th>  Nombre</th><th>Fecha Inicio</th><th>Fecha Fin</th></thead><tbody></tbody></table></div></div></div></div>");
...
//		  }
		...
		function openResourceEditor() {
		  var editor = $("<div>");
		  editor.append("<h2>Lista de Recursos</h2>");
		  editor.addClass("resEdit");
                                        
		  for (var i in ge.resources) {
		    var res = ge.resources[i];
		    var inp = $("<input type='text' readonly='readonly'>").attr("pos", i).addClass("resLine").val(res.name);
		    editor.append(inp).append("<br>");
		  }
