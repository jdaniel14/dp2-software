...
function obtenCostosIndirectos(/*idProyecto,*/tipo){
	...
	$.ajax({
		type: 'GET',
		url: rootURL + 'CO_obtenerCostosIndirectosRealesMes/'+JSON.stringify(obj),		
		dataType: "json",
		async: true,
...
function agregaFilaconRecursos(tipo,i,costoIndirecto,idmoneda, nombreMoneda,codmes, nombreMes){
	...
	muestraTodaFilas("tablaIndirectos");
...
function creaInputMoneda(num, deshabilitado){
	...
	if (deshabilitado=='1')
		combo='<select class="form-control" id="comboMoneda'+num+'"  readOnly disabled >'+ comboMoneda + '</select>';
	else
		combo='<select class="form-control" id="comboMoneda'+num+'"  readOnly disabled >'+ comboMoneda + '</select>';
	return combo;
...
function enviaDatos(obj){
	...
	$.ajax({
		type: 'POST',
		url: rootURL + 'CO_enviarCostosIndirectosRealesMes',
