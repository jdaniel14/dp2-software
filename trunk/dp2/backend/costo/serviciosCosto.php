<?php
	...
	*/
	...
	function CO_obtenerPV($idProyecto, $fecha) { //Valor planeado
		...
		SELECT
		A.ID_ACTIVIDAD,
		SUM(IFNULL(B.CANTIDADESTIMADA,0)*IFNULL(C.COSTO_UNITARIO_ESTIMADO,0)*IFNULL(D.CAMBIO_A_SOL,0)*(DATEDIFF(STR_TO_DATE(:fecha,'%Y%m%d'),A.FECHA_PLAN_INICIO)/DATEDIFF(A.fecha_plan_fin,A.fecha_plan_inicio))) COSTO_PLANEADO_SOLES
		FROM
		ACTIVIDAD A JOIN ACTIVIDAD_X_RECURSO B ON A.ID_ACTIVIDAD=B.ID_ACTIVIDAD
		JOIN RECURSO C ON B.ID_RECURSO=C.ID_RECURSO
		JOIN CAMBIO_HISTORICO D ON C.ID_CAMBIO_MONEDA=D.ID_CAMBIO_MONEDA
	...
	function CO_obtenerEV($idProyecto, $fecha) { //Valor ganado
		...
		SELECT
		A.ID_ACTIVIDAD,
		SUM(IFNULL(B.CANTIDADESTIMADA,0)*IFNULL(C.COSTO_UNITARIO_ESTIMADO,0)*IFNULL(D.CAMBIO_A_SOL,0)*IFNULL(A.PORC_AVANCE_COSTO_ESTIMADO/100,0)) COSTO_PLANEADO_SOLES
		FROM
		ACTIVIDAD A JOIN ACTIVIDAD_X_RECURSO B ON A.ID_ACTIVIDAD=B.ID_ACTIVIDAD
		JOIN RECURSO C ON B.ID_RECURSO=C.ID_RECURSO
		JOIN CAMBIO_HISTORICO D ON C.ID_CAMBIO_MONEDA=D.ID_CAMBIO_MONEDA
